...
export const generateInsight = async (userId, metricDate, regenerate = false) => {
    ...
    const context = {
        ...
        derived: {
            ...
            activeIdleRatio: totalIdle + totalActive > 0 ? Number(((totalActive / Math.max(totalIdle + totalActive, 1)) * 100).toFixed(1)) : null
        }
    };
...
const deriveTags = (metric) => {
    const tags = [];
    if ((metric.totals?.activeMinutes ?? 0) > 240)
        tags.push('marathon');
    if ((metric.totals?.scrollDistance ?? 0) > 5000)
        tags.push('deep-dive');
    if ((metric.totals?.idleMinutes ?? 0) < 30)
        tags.push('laser-focus');
    return tags;
};
const computeMetricSignature = (context) => {
    const significant = {
        date: context.date,
