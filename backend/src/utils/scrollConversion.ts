 ...
 */
export const formatScrollDistance = (pixels: number): { value: string; unit: string; cm: number; km: number } => {
  if (!Number.isFinite(pixels) || pixels < 0) {
 ...
 */
export const formatScrollDistanceWithBoth = (pixels: number): string => {
  if (!Number.isFinite(pixels) || pixels < 0) {
    return '0 cm';
  }
 ...
 */
export const getScrollDistanceDescription = (pixels: number): string => {
  const km = pixelsToKilometers(pixels);
  const cm = pixelsToClimbometers(pixels);
 ...
 */
export const isValidScrollDistance = (value: unknown): boolean => {
  return typeof value === 'number' && Number.isFinite(value) && value >= 0;
};

/**
 * Type for scroll distance data with all conversions
 */
export interface ScrollDistanceData {
  pixels: number;
  centimeters: number;
  kilometers: number;
  formatted: string;
  formattedWithBoth: string;
  description: string;
}
 ...
 */
export const getScrollDistanceData = (pixels: number): ScrollDistanceData => {
  const cm = pixelsToClimbometers(pixels);
  const km = pixelsToKilometers(pixels);
