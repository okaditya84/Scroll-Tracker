...
const buildOtpEmail = (purpose: OtpPurpose, code: string) => {
...
export const issueOtp = async ({ email, purpose, metadata, context, ttlMinutes }: IssueOtpOptions) => {
...
const failWithStatus = async (otp: OtpCodeDocument, status: 'expired' | 'locked') => {
  otp.status = status;
  await otp.save();
};

export const verifyOtp = async ({ email, purpose, code }: VerifyOtpOptions) => {
  const normalizedEmail = email.toLowerCase().trim();
...
export const buildOtpMetrics = async (windowDays = 14) => {
  ...
  totalsRaw.forEach(entry => {
    const purposeKey = entry._id.purpose as string;
    const statusKey = entry._id.status as string;
    totals[purposeKey] = totals[purposeKey] ?? {};
    totals[purposeKey][statusKey] = entry.count;
  });

  const dailyMap = new Map<string, { day: string; signup: Record<string, number>; password_reset: Record<string, number> }>();
  dailyRaw.forEach(entry => {
    const day = entry._id.day as string;
    const purposeKey = entry._id.purpose as string;
    const statusKey = entry._id.status as string;
    ...
    bucket[purposeKey as 'signup' | 'password_reset'][statusKey] = entry.count;
